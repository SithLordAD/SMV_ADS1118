 ...
 */
#define SMV_ADC_CHANNEL_0	0b100 << ADC_CHAN_SHIFT
#define SMV_ADC_CHANNEL_1	0b101 << ADC_CHAN_SHIFT
#define SMV_ADC_CHANNEL_2	0b110 << ADC_CHAN_SHIFT
#define SMV_ADC_CHANNEL_3	0b111 << ADC_CHAN_SHIFT
...
#define ADC_FACTOR_CALC (4.096*2)/32767
...
typedef struct {
	...
	double (*readAdc)(SMV_ads1118);
	double (*getAdc)(SMV_ads1118);
	uint16_t (*checkChannel)(SMV_ads1118);
	uint8_t (*checkFlag)(SMV_ads1118);

	/* data (largest alignment first) */
    double adc_value;
	union uintToInt spi_buf; /* both variables in union 16 bits*/
	int16_t adc_cast;
